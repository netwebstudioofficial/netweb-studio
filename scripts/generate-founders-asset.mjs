...
async function createFoundersVisual() {
  ...
  const target1 = path.resolve('public/WhatsApp Image 2026-09-24 at 6.22.22 PM.jpeg');
  const target2 = path.resolve('public/assets/founders.jpg');
  const target3 = path.resolve('public/assets/founders-vision.jpg');

  fs.writeFileSync(target1, compositeBuffer);
  fs.writeFileSync(target2, compositeBuffer);
  fs.writeFileSync(target3, compositeBuffer);

  console.log('Saved founders image to:');
