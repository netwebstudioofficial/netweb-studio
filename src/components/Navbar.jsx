    ...
    <>
      <header
        className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
          isScrolled
            ? "bg-[#080A08]/90 backdrop-blur-md border-b border-white/[0.08] py-3 shadow-2xl"
            : "bg-transparent border-b border-transparent py-4 sm:py-4.5"
        }`}
        id="main-navigation"
      >
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex items-center justify-between">
          {/* Logo - Centered in middle at all times */}
          <Link
            to="/"
            className="inline-flex items-center justify-center focus:outline-none focus-visible:ring-1 focus-visible:ring-[#A3FF12] rounded transition-opacity duration-200 hover:opacity-90 shrink-0 h-9 sm:h-10 my-auto"
            aria-label="NETWEB STUDIO Home"
          >
            <img
              src={netwebStudioLogo}
              alt="NETWEB STUDIO"
              className="w-[140px] sm:w-[160px] md:w-[190px] h-auto max-h-7 sm:max-h-8 md:max-h-9 object-contain block my-auto"
              loading="eager"
              decoding="async"
            />
          </Link>
